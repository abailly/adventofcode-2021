...
fn num_bits(bytes: &[u8], offset: usize) -> usize {
    bytes.len() * 8 - offset
}
...
fn parse_operator_0(input: (&[u8], usize)) -> IResult<(&[u8], usize), Packet> {
    ...
    match op_prefix(input) {
        Ok(((bytes, off), (version, t, _, len))) => {
            println!("Parsing operator 0, len: {}", len);
            let mut consumed = 0;
            let mut pkts = vec![];
            ...
            while consumed < len {
                match parse_packet(inp) {
                    Ok(((nbytes, noff), p)) => {
                        consumed = consumed + (num_bits(inp.0, inp.1) - num_bits(nbytes, off));
                        println!("Parsed packet: {:?}, consumed: {}", p, consumed);
                        pkts.push(p);
                        inp = (nbytes, noff);
                    }
                    Err(e) => return Err(e),
            ...
            println!("Parsed operator 0, len: {}, consumed: {}", len, consumed);
            Ok((
                inp,
                Packet {
                    version,
...
fn parse_operator_1(input: (&[u8], usize)) -> IResult<(&[u8], usize), Packet> {
    ...
    match op_prefix(input) {
        Ok((inp, (version, t, _, len))) => {
            println!("Parsing operator 1, len: {}", len);
...
pub fn decode_hex(s: &str) -> Vec<u8> {
    if s.len() % 2 != 0 {
        ...
        padded.push('0');
        let mut result = hex::decode(&padded).unwrap();
        result.pop();
        result
    } else {
        hex::decode(s).unwrap()
